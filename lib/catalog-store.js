...
export async function ensureSeedProducts() {
  ...
  const existing = await Product.find({ id: { $in: [...seedById.keys()] }, contentSeedVersion: { $lt: CONTENT_SEED_VERSION } }).lean();
  const ops = [];
  for (const row of existing) {
    ...
    ops.push({ updateOne: { filter: { id: row.id, contentSeedVersion: { $lt: CONTENT_SEED_VERSION } }, update: { $set: set } } });
  }
