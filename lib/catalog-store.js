...
export async function ensureSeedProducts() {
  ...
  const existing = await Product.find({ id: { $in: [...seedById.keys()] } }).select('id background info heroImage editorialCaption faq seo').lean();
  const ops = [];
  for (const row of existing) {
    const seed = seedById.get(row.id); if (!seed) continue;
    const set = {};
    if (row.background === undefined) set.background = seed.background;
    if (row.info === undefined) set.info = seed.info;
    if (row.heroImage === undefined) set.heroImage = seed.heroImage;
    if (row.editorialCaption === undefined) set.editorialCaption = seed.editorialCaption;
    if (row.faq === undefined) set.faq = seed.faq;
    if (row.seo === undefined) set.seo = seed.seo;
    if (Object.keys(set).length) ops.push({ updateOne: { filter: { id: row.id }, update: { $set: set } } });
  }
  if (ops.length) await Product.bulkWrite(ops, { ordered: false }).catch(() => {});
  return true;
}
