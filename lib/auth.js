...
async function syncRegisteredUser(session, login = false) {
  try {
    if (!session?.user?.id) return session;
    const db = await connectToDatabase();
    if (!db.connected) return session;
    const now = new Date();
    const set = {
      email: session.user.email || '',
      name: session.user.name || session.user.email || 'Vásárló',
      ssoRole: session.permission?.role || 'user',
      ssoStatus: session.permission?.status || 'unknown',
      active: true,
      lastSeenAt: now
    };
    if (login) set.lastLoginAt = now;
    const account = await User.findOneAndUpdate(
      { ssoUserId: session.user.id },
      { $set: set, $setOnInsert: { firstLoginAt: now } },
      { upsert: true, new: true, setDefaultsOnInsert: true }
    ).lean();
    if (account?.roleOverride) {
      return { ...session, permission: { ...session.permission, status: 'approved', role: account.roleOverride } };
    }
    return session;
  } catch {
    return session;
...
export async function completeOAuth(request, redirectUri) {
  ...
  let session = { user: { id: user.sub, email: user.email || '', name: user.name || user.email || 'Vásárló' }, permission: { status: permission?.status || 'unknown', role: permission?.role || user.role || 'user' }, expiresAt: Date.now() + SESSION_MAX_AGE * 1000 };
  session = await syncRegisteredUser(session, true);
...
export async function getSession() {
  try {
    ...
    return await syncRegisteredUser(session, false);
  } catch { return null; }
}
