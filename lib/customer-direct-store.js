...
async function customerDirectRequest(path,{method='GET',body}={}){
 ...
 const response=await fetch(url,{method,headers:{'content-type':'application/json','x-forwarded-host':config.host},body:body?JSON.stringify(body):undefined,cache:'no-store'});
...
export async function getCustomerDirectSiteSettings(fallback){
 ...
 return {
  ...fallback,
  sales:{...fallback.sales,checkoutEnabled:true,supportEmail:storefront.settings?.supportEmail||fallback.sales?.supportEmail,senderName:storefront.settings?.companyName||fallback.sales?.senderName},
