...
export default function DashboardClient({ user }) {
  ...
  return <main className="dashboard-shell">
    ...
    <section className="metric-grid">
      <article className="metric"><span className={`status-dot ${mongoOk ? 'ok' : 'warn'}`} /><small>MongoDB</small><strong>{health ? health.mongo.state : 'Ellenőrzés…'}</strong><p>{health?.mongo?.configured === false ? 'MONGODB_URI nincs beállítva.' : 'Adatbázis-kapcsolat állapota.'}</p></article>
