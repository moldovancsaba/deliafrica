...
export default function UsersClient() {
  ...
  return <>
    <div className="admin-page-head"><div><span className="admin-kicker">USER MANAGEMENT</span><h1>Felhasználók</h1><p>Regisztrált vásárlók, aktivitás és admin jogosultságok kezelése.</p></div></div>
    ...
    <section className="admin-panel">
      ...
      {loading ? <p>Betöltés…</p> : <div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Felhasználó</th><th>Szerepkör</th><th>Regisztráció / aktivitás</th><th>Rendelések</th><th>Jogosultság</th></tr></thead><tbody>
        {filtered.map(user => <tr key={user.ssoUserId}>
          <td><b>{user.name || 'Névtelen felhasználó'}</b><br/><span className="muted">{user.email || 'Nincs e-mail'}</span><br/><small className="muted">{user.ssoUserId}</small>{user.isCurrentUser && <><br/><span className="status-pill ok">Te</span></>}</td>
          ...
          <td><b>{user.orderCount || 0} db</b><br/><small>{money(user.totalSpent)}</small><br/><small>{user.lastOrderAt ? `Utolsó: ${date(user.lastOrderAt)}` : 'Nincs rendelés'}</small></td>
