...
import { DEFAULT_STOREFRONT_CONTENT, DEFAULT_UI_COPY } from '@/lib/site-config';
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    fetch('/api/site-settings', { cache: 'no-store' }).then((res) => res.json()).then((data) => {
      ...
      setUiCopy(data.uiCopy || DEFAULT_UI_COPY);
  ...
  return <main>
    ...
    <section className="story-block" id="story">
      ...
      <div className="story-copy">{storefrontContent.storyBody.split(/\n\n+/).map((paragraph) => <p key={paragraph}>{paragraph}</p>)}<a className="story-link" href="#shop">{uiCopy.story.link}</a></div>
    </section>
    ...
    <section className="why-block" id="why">
      ...
      <div className="why-grid">{(uiCopy.why.cards || []).map(card => <article key={card.number}><b>{card.number}</b><h3>{card.title}</h3><p>{card.body}</p></article>)}</div>
    </section>
    ...
    <section className="shop" id="shop">
      ...
      <div className="product-grid">{shown.map(product => <article className="product-card" key={product.id}>
        ...
        <div className="product-info"><small>{product.subtitle}</small><h3>{product.name}</h3><p>{product.story}</p><div className="product-bottom"><strong>{formatPrice(product.price)}</strong><button disabled={product.price == null} onClick={() => add(product.id)}>{product.price == null ? uiCopy.shop.unavailable : '+'}</button></div></div>
      </article>)}</div>
    </section>
    ...
    {detail && <div className="modal-backdrop" onClick={() => setDetail(null)}><div className={`detail-modal category-${detail.category}`} onClick={e => e.stopPropagation()}>
      ...
      <div><span className="badge inline">{detail.badge}</span><h2>{detail.name}</h2><p>{detail.story}</p><strong>{formatPrice(detail.price)}</strong><div className="modal-actions">{detail.price != null && <button className="button button-red" onClick={() => { add(detail.id); setDetail(null); }}>{uiCopy.modal.addToCart}</button>}<Link className="button button-outline" href={`/products/${detail.slug}`}>{uiCopy.modal.knowMore}</Link></div></div>
    </div></div>}

    <aside className={`cart-drawer ${cartOpen ? 'open' : ''}`}>
      <div className="drawer-head"><h2>{uiCopy.cart.title}</h2><button onClick={() => setCartOpen(false)}>×</button></div>
      {cartRows.length === 0 ? <div className="empty-cart">{uiCopy.cart.empty}<button className="button button-dark" onClick={() => setCartOpen(false)}>{uiCopy.cart.continueShopping}</button></div> : <>
        ...
        <div className="cart-total"><span>{uiCopy.cart.total}</span><strong>{formatPrice(total)}</strong></div>
        <button className="button button-red full" disabled={!checkoutEnabled} onClick={beginCheckout}>{!checkoutEnabled ? uiCopy.cart.disabled : session?.authenticated || session?.configured === false ? uiCopy.cart.checkout : uiCopy.cart.loginCheckout}</button>
      </>}
    </aside>
    {cartOpen && <button className="drawer-overlay" onClick={() => setCartOpen(false)}/>}
    ...
    {checkout && <div className="modal-backdrop"><form className="checkout-modal" onSubmit={placeOrder}>
      ...
      <span className="eyebrow dark">{uiCopy.checkout.title}</span><h2>{uiCopy.checkout.title}</h2>
      <div className="form-grid">
        <label>{uiCopy.checkout.name}<input required value={form.customerName} onChange={e => setForm({ ...form, customerName: e.target.value })}/></label>
        <label>{uiCopy.checkout.email}<input required type="email" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })}/></label>
        <label>{uiCopy.checkout.phone}<input value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value })}/></label>
        <label className="wide">{uiCopy.checkout.address}<textarea required value={form.address} onChange={e => setForm({ ...form, address: e.target.value })}/></label>
      </div>
      <div className="cart-total"><span>{uiCopy.cart.total}</span><strong>{formatPrice(total)}</strong></div>
      <button className="button button-red full" disabled={orderState?.loading}>{orderState?.loading ? `${uiCopy.checkout.submit}…` : uiCopy.checkout.submit}</button>
      {orderState && !orderState.loading && !orderState.ok && <p className="form-error">{orderState.error}</p>}
    </form></div>}

    {orderState?.ok && <div className="order-toast"><b>{uiCopy.checkout.success}</b><span>{orderState.reference}</span><small>{orderState.message}</small><button onClick={() => setOrderState(null)}>×</button></div>}
  </main>;
}
